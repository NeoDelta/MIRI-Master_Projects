...
class GLWidget : public QGLWidget {
 ...
 private:
  ...
  /**
   * @brief program_ The ssao shader program.
   */
  std::unique_ptr<QOpenGLShaderProgram> ssao_program_;

  /**
   * @brief program_ The 3Dssao shader program.
   */
  std::unique_ptr<QOpenGLShaderProgram> ssao3D_program_;

  /**
   * @brief program_ The separable ssao shader pass program.
   */
  std::unique_ptr<QOpenGLShaderProgram> sao_program_;

  /**
   * @brief program_ The blur shader pass program.
   */
   ...
   */
  std::vector<std::unique_ptr<data_representation::TriangleMesh>> mesh_;

  std::vector<data_representation::TriangleMesh> LODmeshes_;

  std::vector<Eigen::Vector4f> meshInstances;

  std::vector<Eigen::Vector3f> walls;
  ...
  /**
   * @brief metalness_ Parameters for SSAO.
   */
  int kernelSize_;
  int intensity_;
 ...
 protected slots:
  ...
  /**
   * @brief SetReflection Enables the 3D SSAO shader.
   */
  void Set3DSSAO(bool set);

  /**
   * @brief SetReflection Enables the separable SSAO shader.
   */
  void SetSAO(bool set);
  ...
  void SetKernelSize(double);

  void SetAOIntensity(double);

  void SetSampleRadius(double);

  void SetRange(double);

  void LoadTileMap();

  void CriticalRendering(float maxTriangles);
